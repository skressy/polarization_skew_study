...
def wavy_field_gs95(M_A, Bz_true, amplitude_range, g_size, anisotropy_strength=1.0, seed=None, plotting=False):
    ...
    for iz in range(g_size):
        ...
        Bx_bg[:, :, iz] = Bx0 * np.cos(theta0)
        By_bg[:, :, iz] = By0 * np.sin(theta0)
        Bz_bg[:, :, iz] = Bz_true
    ...
    if plotting:
        ...
        P_k, phi_k, cg_k = wavy_field_kolmo(M_A, Bz_true, amplitude_range, g_size, plotting=False)
...
def wavy_field_random(M_A, Bz_true, amplitude_range, g_size, plotting=False):
    """
