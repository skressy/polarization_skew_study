...
# def _geometry_uniform(X, Y, Z, alpha, iaxis=2, Bz_amp=0.0, **_):
#     """
#     Uniform POS field at angle alpha in the XY plane.
#     Bx = cos(alpha), By = sin(alpha), Bz = Bz_amp (optional LOS component)
#     b_hat : along LOS — avoids stripe artefacts from POS-aligned anisotropy axis
#     B0_ref: 1.0 (unit field)
#     Bz_amp : float, optional, Amplitude of LOS (Zeeman) component. Default 0.0 (purely POS field).
#     """
#     Bx0 = np.full_like(X, np.cos(alpha))
#     By0 = np.full_like(X, np.sin(alpha))
#     Bz0 = np.full_like(X, Bz_amp)

#     b_hat  = _los_b_hat(iaxis)
#     B0_ref = 1.0

#     # Synthetic Zeeman: mean of Bz along LOS axis (uniform density assumption)
#     B_LOS  = np.mean(Bz0, axis=iaxis)

#     return Bx0, By0, Bz0, b_hat, B0_ref, B_LOS
...
# With Noise
def _stokes_and_polarization(Bx, By, Bz, iaxis, sigma_noise=0.0, seed=None):
    ...
    if sigma_noise > 0.0:
        rng     = np.random.default_rng(seed)
        sigma_I = sigma_noise * I_obs          # scale noise with depth
        Q_obs   = Q_obs + rng.normal(0.0, sigma_I, size=Q_obs.shape)
        U_obs   = U_obs + rng.normal(0.0, sigma_I, size=U_obs.shape)

    # ── Compute P with Rice debiasing ─────────────────────────────────────
    P_raw = np.sqrt(Q_obs**2 + U_obs**2) / (I_obs + 1e-12)

    if sigma_noise > 0.0:
        sigma_P   = sigma_noise                        # noise on normalised P
        P_debiased = np.sqrt(np.maximum(P_raw**2 - sigma_P**2, 0.0))
        low_snr    = P_raw < sigma_P                   # SNR < 1: mask
        P_debiased[low_snr] = np.nan
        P_obs = P_debiased
    else:
        P_obs = P_raw                                  # no debiasing needed

    phi_obs = 0.5 * np.arctan2(U_obs, Q_obs)

    return P_obs, phi_obs, cos2g.mean(axis=iaxis) #, Q_obs, U_obs
...
def simulate_field(geometry='helical', turbulence='gs95', M_A=1.0, alpha=0.0, iaxis=2, g_size=64,
    ...
    # --- Stokes / polarization ---
    return _stokes_and_polarization(Bx, By, Bz, iaxis, sigma_noise=snoise, seed=None)


def compute_S(phi_map, delta_pixels=2):

    ny, nx = phi_map.shape

    phi_map = np.degrees(phi_map)

    # Build pixel offsets within circular aperture of radius delta
    r = delta_pixels
    offsets = [
        (dy, dx)
        for dy in range(-r, r + 1)
        for dx in range(-r, r + 1)
        if 0 < np.sqrt(dy**2 + dx**2) <= r]

    sum_sq = np.zeros_like(phi_map)
    count  = np.zeros_like(phi_map)

    for dy, dx in offsets:
        # Compute valid index ranges (no wrapping — skip out-of-bounds neighbors)
        y0, y1 = max(0, -dy),  min(ny, ny - dy)
        x0, x1 = max(0, -dx),  min(nx, nx - dx)
        sy0, sy1 = y0 + dy, y1 + dy
        sx0, sx1 = x0 + dx, x1 + dx

        d_chi = phi_map[y0:y1, x0:x1] - phi_map[sy0:sy1, sx0:sx1]

        # Wrap PA difference to [-90, 90] using % operator
        d_chi = (d_chi + 90) % 180 - 90

        sum_sq[y0:y1, x0:x1] += d_chi**2
        count[y0:y1,  x0:x1] += 1

    valid = count > 0
    S_map = np.zeros_like(phi_map)
    S_map[valid] = np.sqrt(sum_sq[valid] / count[valid])
